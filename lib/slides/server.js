...
Server.prototype.initDb = function() {
  ...
  // Randomize slug
  models.Slideshow.pre('save', function(next) {
  ...
  models.Slideshow.method({
    broadcast: function(action, data) {
      ...
      that.redis.publish('slideshow.' + id, data);

      if (id != this.slug.toString()) {
        that.redis.publish('slideshow.' + this.slug, data);
      }
    }
  });
  ...
  models.Slide.pre('save', broadcastAction('update', function(slide) {
    return {
      index: slide.index,
  ...
  models.Slide.pre('remove', broadcastAction('delete', function(slide) {
    return slide.index;
  }));
